...
const PDF_PATH = '../report/archive/composer_diversity_report.pdf';

const GRAPH_CREDIT_TEXT = '© Sandrine Rudaz — Composer Diversity Report';
...
function appendGraphCredit(target) {
  if (!target || target.querySelector('.graph-credit')) return;
...
function initGraphCredits() {
  ...
  document.querySelectorAll('.share-bar').forEach((bar) => {
    if (!bar.querySelector('.graph-credit')) {
      const credit = document.createElement('p');
      credit.className = 'graph-credit';
      credit.textContent = GRAPH_CREDIT_TEXT;
...
function buildDotMatrixSVG() {
  ...
  return `
    <svg role="img" aria-label="Dot matrix showing about four of one hundred credits" viewBox="0 0 ${width} ${height}">
      <title>Women composer share, 2000-2017</title>
...
function initCitation() {
  ...
  const title = document.querySelector('.brand')?.textContent?.trim() || 'Composer Diversity Report';
